...
face_cascade = cv2.CascadeClassifier(
    cv2.data.haarcascades + "haarcascade_frontalface_default.xml")  # Front face
pface_cascade = cv2.CascadeClassifier(
    cv2.data.haarcascades + "haarcascade_profileface.xml")        # Profile face
...
while True:
    ...
    faces = face_cascade.detectMultiScale(
        gray, scaleFactor=1.2, minNeighbors=5, minSize=(30, 30))  # Front Face
    pfaces1 = pface_cascade.detectMultiScale(
        gray, scaleFactor=1.2, minNeighbors=5, minSize=(30, 30))  # Left face
    flipgray = cv2.flip(gray, 1)
    pfaces2 = pface_cascade.detectMultiScale(
        flipgray, scaleFactor=1.2, minNeighbors=5, minSize=(30, 30))  # Right face
    ...
    for (x, y, w, h) in pfaces2:   # Right Side face
        cv2.rectangle(image, (vwidth-x, y), (vwidth-x-w, y+h),
                      (0, 0, 255), 2)  # Red
